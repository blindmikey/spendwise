...
| `envelope` | −    | mini savings account: monthly allotment (`value`), accumulated balance (`avail`). Spending is **derived** - the sum of expense rows assigned to the envelope. Overspend counts against the month it happens, once. |
...
- **Close Out** finalizes a month: envelope balances roll over
  ...
  recorded, and the next month is created.
...
- **Keyboard**: `Ctrl+S` saves (month or settings), `Ctrl+Z` / `Ctrl+Y` (or
  ...
  groups rapid keystrokes, and resets on close-out/recompute.
...
npm test             # engine, merge, api-core, web server, importer (vitest)
